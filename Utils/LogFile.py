...
class LogFile:
    ...
    def __init__(self, filename, delim, header, rows, time_attr, trace_attr, activity_attr = None, values = None, integer_input = False, convert = True, k = 1, dtype=None):
        ...
        self.time = time_attr # "completeTime"
        self.trace = trace_attr # case
        self.activity = activity_attr # event
        ...
        if filename is not None:
            ...
            for _, col_type in self.data.dtypes.items():
                if col_type == 'float64':
                   self.numericalAttributes.add(col_type)
                else:
                    self.categoricalAttributes.add(col_type)
    ...
    def convert_column2ints(self, x):
        ...
        if x.name not in self.values:
            ...
            return y + 1 # порядковые номера уникальных значений признака
        else:
            x = x.astype("str")
    ...
    def create_trace_attribute(self):
        ...
        result = map(self.create_trace_attribute_case, self.data.groupby([self.trace]))    
    ...
    def create_k_context(self):
        ...
        # print('self.contextdata: ', self.contextdata)
    ...
    def create_k_context_trace(self, trace):
        ...
        # print('shifted: ', trace_data.shift())
        # shift_data = trace_data.shift().infer_objects(copy=False).fillna(0)
        pd.set_option('future.no_silent_downcasting', True)
        
        shift_data = trace_data.shift().fillna(0).infer_objects(copy=False)
        # print(trace[0])
        # print(self.trace)
        # print(shift_data)
        # print(shift_data.at[shift_data.first_valid_index(), self.trace])
        shift_data.at[shift_data.first_valid_index(), self.trace] = trace[0]
        joined_trace = shift_data.join(trace_data, lsuffix="_Prev0")
        for i in range(1, self.k):
            shift_data = shift_data.shift().infer_objects(copy=False).fillna(0)
            shift_data.at[shift_data.first_valid_index(), self.trace] = trace[0]
            joined_trace = shift_data.join(joined_trace, lsuffix="_Prev%i" % i)
        # contextdata = contextdata.append(joined_trace, ignore_index=True)
        contextdata = df = pd.concat([contextdata, joined_trace], ignore_index=True)
        contextdata = contextdata.astype("int", errors="ignore")
        # print('k suffixes: ', contextdata)
        file_out = '../k_suffixes.csv'
        contextdata.to_csv(file_out, index=False)
        return contextdata
    ...
    def add_end_events(self):
        ...
        result = map(self.add_end_event_case, cases)
