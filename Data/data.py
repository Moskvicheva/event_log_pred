...
class Data:
    ...
    def prepare(self, setting):
        ...
        if setting.prefixsize:
            ...
            prefix_size = max(self.logfile.get_cases().size()) # tax: 15
            if prefix_size > 40:
                self.logfile.k = 40
            else:
        ...
        if setting.train_split != "k-fold":
            ...
            self.train.contextdata = self.train.contextdata.sort_values(by=[self.train.time]).reset_index(drop=True)

            self.test_orig.contextdata = self.test_orig.contextdata.sort_values(by=[self.train.time]).reset_index(drop=True)
        else:
